...
DATABASE = "printer_system.db"

# 3. القاموس اللغوي (تم إضافة الرمز والملاحظات والتعديل)
LANGS = {
    'ar': {
...
def init_db():
    with get_db() as conn:
        # إضافة code و notes في إنشاء الجدول الجديد
        ...
        # كود لحماية بياناتك السابقة: سيقوم بإضافة الحقول الجديدة إن لم تكن موجودة في قاعدة بياناتك القديمة
...
def index():
    ...
    with get_db() as conn:
        query_sql = f"%{q}%"
        # تم تضمين الرمز والملاحظات في البحث
...
def add():
    if session.get('role') in ['admin', 'entry']:
        with get_db() as conn:
            try:
                # تضمين الحقول الجديدة في دالة الإضافة
